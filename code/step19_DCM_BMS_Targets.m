...
parameters.saveDir   = fullfile(parameters.modelDir, 'Targets');
...
for s = 1 : size(subjID,1)
    models = cellstr(spm_select('FPList', fullfile(p.modelDir, subjID{s}), ['^DCM_' subjID{s} '.*\.mat$'])); 
...
matlabbatch{1}.spm.dcm.bms.inference.family_level.family(1).family_name     = 'Pref';
matlabbatch{1}.spm.dcm.bms.inference.family_level.family(1).family_models   = [1:4]';

matlabbatch{1}.spm.dcm.bms.inference.family_level.family(2).family_name     = 'NonPref';
